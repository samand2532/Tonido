    ...
    for KetsU = 1:length(AllBras)
        ...
        for rr = 1:length(APA);
            ...
            if InitialBraU == FinalKetU
                ...
                OperatorConst = m1Const * m2Const * m3Const *m4Const;
                ...
                PiTerm = sqrt((factorial(nk1)*factorial(nk2)*factorial(nk3)*factorial(nk4))/(factorial(nk1+abs(m1))*factorial(nk2+abs(m2))*factorial(nk3+abs(m3))*factorial(nk4+abs(m4))));
                MomTerm = 1/(2^((dot(InitialBraU,MomValue))/2));
