...
Nmat = zeros(NoOfVectsLookedAt);%zeros(39);
Lmat = zeros(NoOfVectsLookedAt);%zeros(39);
UmatNoConst = zeros(NoOfVectsLookedAt);%zeros(39);
Vmat = zeros(NoOfVectsLookedAt);%zeros(39);

%%%%%%NTerm Follows
NMatFinal = eye(NoOfVectsLookedAt).*N;
%%%%%%%NTermEnd%%%%

%%%%%L Term Begin%%%%
for BraL = 1:NoOfVectsLookedAt;%length(AllBras)
    for KetL = 1:NoOfVectsLookedAt;%length(AllBras)
...
ee=0;
MomTest=zeros(200,1);
for BrasU = 1:NoOfVectsLookedAt;%length(AllKets)
    for KetsU = 1:NoOfVectsLookedAt;%length(AllBras)
        ...
        for rr = 1:length(APA);
            ...
            if InitialBraU == FinalKetU
                ...
                OperatorConst = m1Const * m2Const * m3Const *m4Const% CHECKED!!!!!
                I2 = factorial(((abs(m1-2)) + (abs(m2-2)) + (abs(m3-2)) + (abs(m4-2)))/2);%checked
                ...
                PiTerm = sqrt((factorial(nk1)*factorial(nk2)*factorial(nk3)*factorial(nk4))/(factorial(nk1+abs(m1-2))*factorial(nk2+abs(m2-2))*factorial(nk3+abs(m3-2))*factorial(nk4+abs(m4-2))));
                MomTerm = 1/(2^(dot(InitialBraU,abs(MomValue))/2));
                ee = ee+1;
                MomTest(ee,1) = MomTerm;
                TotalU = MomTerm*I2*PiTerm*OperatorConst;
                UmatNoConst(BrasU,KetsU) = UmatNoConst(BrasU,KetsU) + TotalU;
            end
        end
    end
end
